...
from app.llm.config import LLM_CONFIG, CURRENT_MODEL
from app.llm.pipeline import run_query, invalidate_cache
...
# ── 인프라 연결 ────────────────────────────────────────────────────────────
try:
    engine = create_engine(os.environ.get("DATABASE_URL", "postgresql://postgres:8342@localhost:5432/pdf_db"))
except Exception as e:
    print(f"[DB] 연결 실패: {e}"); engine = None

try:
    redis_client = redis.from_url(os.environ.get("REDIS_URL", "redis://localhost:6379"))
except Exception as e:
    print(f"[Redis] 연결 실패: {e}"); redis_client = None
...
def health():
    ...
    if redis_client:
        try:
            redis_client.ping(); status["redis"] = "ok"
        except Exception as e:
...
async def upload_and_summarize(
    ...
    doc_id: int = Form(...),
    user_id: int = Form(...),
    category: str = Form(default="기타"),   # 법령·조례 / 가이드라인·지침 / 공모·사업 / 감사 / 내부 규정 / 기타
):
    """
    ...
    try:
        ...
        ingest_result = await loop.run_in_executor(
            executor,
            lambda: ingest_pdf(tmp, doc_id, user_id,
                               doc_name=file.filename, category=category),
        )
        ...
        try:
            result = await loop.run_in_executor(
                ...
                lambda: ingest_markdown(
                    markdown_text, doc_id, user_id, doc_name,
