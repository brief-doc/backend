        ...
        if pdf_doc_pdfium is not None:
            try:
                ...
                text_in_bbox = textpage.get_text_bounded(
                    left=x0, bottom=page_height - y1,
                    right=x1, top=page_height - y0,
                )
                textpage.close()
...
def _collapse_colspan_headers(md: str) -> str:
    ...
    def is_sep_row(cells: list[str]) -> bool:
        return bool(cells) and all(
            not set(c.replace('-', '').replace(':', '').replace(' ', ''))
            for c in cells
        )
    ...
    for i, ln in enumerate(lines):
        ...
        if len(cells) == len(hdr_cells):
            merged = [
                ' '.join(cells[j] for j in idx_list if cells[j].strip())
...
def _merge_split_tables(blocks: list[str]) -> list[str]:
    ...
    for block in blocks:
        if (merged
                and is_table(merged[-1])
                and is_table(block)
                and _col_count(merged[-1]) == _col_count(block)
                and _col_count(block) > 0):
...
def process_docx(file_path: str, page_filter: set = None) -> list[str]:
    ...
    try:
        ...
        for item, level in doc.iterate_items():
            ...
            if _in_table_level is not None and level > _in_table_level and label in (
                "TEXT", "CAPTION", "PARAGRAPH", "DOCUMENT_INDEX"
            ):
                continue
