...
def _extract(data: dict) -> str:
    parts = []
    info = data.get("info", {})
    if v := info.get("caseNm"):    parts.append(f"사건명: {v}")
    if v := info.get("caseTitle"): parts.append(f"판결: {v}")
    for item in data.get("jdgmnInfo", []):
        if q := item.get("question"): parts.append(f"Q: {q}")
        if a := item.get("answer"):   parts.append(f"A: {a}")
    for item in data.get("Summary", []):
        if s := item.get("summ_pass"): parts.append(f"요약: {s}")
    kws = [k.get("keyword") for k in data.get("keyword_tagg", []) if k.get("keyword")]
    if kws: parts.append(f"키워드: {', '.join(kws)}")
    return "\n".join(parts)
    ...
    for idx, f in enumerate(files):
        try:
            ...
            if case_no in existing:
                skip += 1; continue
            ids.append(case_no); texts.append(_extract(data))
            metas.append({"source": f.name, "case_no": case_no, "user_id": 1, "doc_type": "qa_data"})
            new += 1
