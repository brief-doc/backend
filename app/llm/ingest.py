...
from .config import COLLECTION_NAME  # retriever 와 동일 컬렉션 사용
...
def _chroma_vs(collection: str | None = None):
    ...
    return Chroma(persist_directory=CHROMA_DB_PATH,
                  embedding_function=_embeddings(), collection_name=col)
...
# ── PDF 저장 ──────────────────────────────────────────────────────────────────
def ingest_pdf(file_path: str, doc_id: int, user_id: int,
               doc_name: str = None, category: str = "기타") -> dict:
    """
    ...
    return {"status": "success", "file_name": name, "category": category,
            "total_chunks": len(chunks), "total_pages": len(documents)}


# ── PDF 배치 저장 ─────────────────────────────────────────────────────────────
def batch_ingest_pdfs(pdf_folder: str = "./pdf_files",
                      start_doc_id: int = 0, user_id: int = 1) -> dict:
    ...
    for idx, path in enumerate(files):
        ...
        try:
            ...
            for i, c in enumerate(chunks):
                c.metadata.update({"doc_id": doc_id, "user_id": user_id,
...
def _chunk_by_sections(content: str) -> list[dict]:
    ...
    for line in content.split("\n"):
        if line.startswith("#"):
            if cur:
                text = "\n".join(cur).strip()
                if len(text) > 10: chunks.append({"section": section, "content": text})
            section = line.lstrip("#").strip(); cur = [line]
        else:
            cur.append(line)
    if cur:
        text = "\n".join(cur).strip()
...
def _summarize(text: str) -> str:
    try:
        r = requests.post(f"{_OLLAMA_URL}/api/generate",
...
def ingest_markdown(markdown_content: str, doc_id: int, user_id: int,
                    doc_name: str = "마크다운 문서", chunking_method: str = "sections",
                    enable_summary: bool = True, category: str = "기타") -> dict:
    """
    ...
    Args:
        category: 문서 카테고리 (법령·조례 / 가이드라인·지침 / 공모·사업 /
                  감사 / 내부 규정 / 기타)
    """
    chunks = (_chunk_by_sections(markdown_content) if chunking_method == "sections"
              else [{"section": "N/A", "content": c.strip()}
                    for c in _MD_SPLITTER.split_text(markdown_content) if len(c.strip()) > 10])
    if not chunks:
        return {"status": "error", "detail": "청킹 결과 없음"}
    if enable_summary:
        for c in chunks: c["summary"] = _summarize(c["content"])
    try:
        col = _qa_collection()
        col.add(ids=[f"{doc_id}_md_{i}" for i in range(len(chunks))],
...
def save_markdown_to_vector_db(markdown_content: str, doc_id: int, user_id: int,
                                doc_name: str = "마크다운 문서",
                                chunking_method: str = "sections",
                                enable_summary: bool = True) -> dict:
    return ingest_markdown(markdown_content, doc_id, user_id, doc_name, chunking_method, enable_summary)
